...
public class AppTestVisualizzazioneDatiFunzionale 
{
	
	//testa la visualizzazione di un app
	//da documenti so che la visualizzazione deve generare una vista dati 
	//o meglio una stampa
	//(da documenti il metodo dovrebbe richiamare i metodi di estrazionedati,
	//normalizzazione di ogni app del gruppo e aggregazione)
	//siccome non ho il codice dei metodi poichè sono stati sviluppati
	//dai miei colleghi, questi metodi sono sostituiti 
	//dagli assegnamenti qui sotto
	@Test
	public void testVisualizzazioneAppDatiCompleti()
	{
		//creo un app 
		ElementAppStore store=new GoogleStore();
		App appFunzionale=new App("appFunzionale",store);
		...
		d.setPackage_name_overview(appFunzionale.getNomeApp());
		d.setDaily_average_overview(daily2);
		d.setTotal_average_overview(daily2);
		
		//carrier
		d.setData_carrier(date);
		d.setPackage_name_carrier(appFunzionale.getNomeApp());
		...
		d.setPackage_name_country(appFunzionale.getNomeApp());
		...
		d.setPackage_name_device(appFunzionale.getNomeApp());
		d.setDevice(device);
		...
		d.setPackage_name_language(appFunzionale.getNomeApp());
		...
		d.setPackage_name_os_version(appFunzionale.getNomeApp());
		...
		d.setPackage_name_tablet(appFunzionale.getNomeApp());
		...
		d.setPackage_name_app_version(appFunzionale.getNomeApp());
		d.setApp_version(app_version);
		d.setDaily_average_app_version(daily2);
		d.setTotal_average_app_version(daily2);
		appFunzionale.setDati(d);
		appFunzionale.visualizzazioneDati();
	}
